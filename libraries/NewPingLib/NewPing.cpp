...
NewPing::NewPing() {
    ...
    _maxEchoTime = min(MAX_DISTANCE, MAX_SENSOR_DISTANCE) * US_ROUNDTRIP_CM + (US_ROUNDTRIP_CM / 2);
...
/*-----------------------------------------------------------------
 *  Convert ping distance to "string"
 *---------------------------------------------------------------*/
char* NewPing::distToString() {
    char buff[32];
    unsigned int dist = NewPing::ping() / US_ROUNDTRIP_CM;
    sprintf(buff, "%d", dist);
    strcat (buff, "cm");
    return buff;
}

unsigned int NewPing::distToInt() {
    return NewPing::ping() / US_ROUNDTRIP_CM;
}
