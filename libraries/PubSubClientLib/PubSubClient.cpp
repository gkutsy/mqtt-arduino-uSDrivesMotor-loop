/*
 PubSubClient.cpp - A simple client for MQTT.
  ...
  Modified by German Kutsy, Litmus Automation LLC. (2014)
*/
...
// dev version (w/o username,password)
PubSubClient::PubSubClient(uint8_t* ip, uint16_t port, void (*callback)(char*,uint8_t*,unsigned int), Client& client, char* clientId, char* projectId, char* deviceName, char* jsonKV, boolean cleanSession) {
    this->ip = ip;
    this->port = port;
    this->domain = NULL;
    this->callback = callback;
...
// production version (username,password) included
PubSubClient::PubSubClient(uint8_t* ip, uint16_t port, void (*callback)(char*,uint8_t*,unsigned int), Client& client, char* username, char* password,
                           char* clientId, char* projectId, char* deviceName, char* jsonKV, boolean cleanSession) {
    this->ip = ip;
    this->port = port;
    this->domain = NULL;
    this->callback = callback;
...
boolean PubSubClient::connect() {
    return connect(clientId,username,password,0,0,0,0);
}
...
// working code below: copied and changed.
//boolean PubSubClient::publish(char* topic, char* payload) {
//   return publish(topic,(uint8_t*)payload,strlen(payload),false);
//}

// my variation below:
boolean PubSubClient::publish(char* stream, char* payload) {
    char buff[128];
    strcpy (buff, projectId);
    strcat (buff, "/");
    strcat (buff, deviceName);
    strcat (buff, "/");
    strcat (buff, stream);
    return publish(buff,(uint8_t*)payload,strlen(payload),false);
}
...
boolean PubSubClient::subscribe(char* stream) {
   if (connected()) {
      // Leave room in the buffer for header and variable length field
      uint16_t length = 7;
      nextMsgId++;
      if (nextMsgId == 0) {
       ...
       char topicPath[128];
       strcpy (topicPath, projectId);
       strcat (topicPath, "/");
       strcat (topicPath, deviceName);
       strcat (topicPath, "/");
       strcat (topicPath, stream);
      length = writeString(topicPath, buffer,length);
